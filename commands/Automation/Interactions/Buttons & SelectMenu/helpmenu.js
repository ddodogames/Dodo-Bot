...
$ephemeral
    ...
    $title[Entertainment]
    $addField[**Games**;
\`rps\`, \`tictactoe\`, \`findtheflag\`, \`matchpairs\`, \`findemoji\`, \`hangman\`, \`snake\`, \`c4\`, \`2048\`, \`minesweeper\`, \`trivia\`, \`coinflip\`, \`flood\`, \`wyr\`
 ]
   $addField[**Text**;
\`howgamer\`, \`whoasked\`, \`8ball\`, \`say\`, \`reverse\`, \`randomtopic\`, \`joke\`, \`clap\`, \`emojipasta\`, \`mock\`, \`yesorno\`, \`lulcat\`, \`fact\`, \`showerthoughts\`
]
    $addField[**Images**;
\`gun\`, \`jail\`, \`couldread\`, \`nokia\`, \`greyscale\`, \`ad\`, \`drip\`, \`invert\`, \`pet\`, \`wanted\`, \`comment\`, \`whereiseveryone\`, \`achievement\`, \`clown\`
]
    $color[$getGlobalVar[embedcolor]]
    $addActionRow
    $addStringSelectMenu[helpmenu_$authorID;Select a module;false;1;1]
...
\`randomcolor\`, \`avatar\`, \`banner\`, \`morse\`, \`aur\`, \`github\`, \`periodic-table\`, \`suggest\`, \`poll\`]
    $color[$getGlobalVar[embedcolor]]
    $addActionRow
    $addStringSelectMenu[helpmenu_$authorID;Select a module;false;1;1]
    $addOption[Main;Main module;main;;false]
